...
export const RiotWaiter = {
  ...
  */
  ...
  /** Resolve a promise if connection hang up
   *  in order to prevent possible memory leaks
   * @param {Function} resolve - a resolve callback
   */
  memGuard(resolve) {
    if (this.opts.req && this.opts.req.on) {
      this.opts.req.on('close', resolve);
    }
  },
};
